...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune a transformers model on a Masked Language Modeling task")
    parser.add_argument("--dataset_name", type=str, default=None, help="The name of the dataset to use (via datasets library)")
    parser.add_argument("--dataset_config_name", type=str, default=None, help="The configuration name of the dataset to use (via datasets library)")
    ...
    parser.add_argument("--push_to_hub", action="store_true", help="Whether or to push the model to the Hub.")
    parser.add_argument("--hub_model_id", type=str, help="The name of the repository to keep in sync with the local `output_dir`.")
    parser.add_argument("--hub_token", type=str, help="The token to use to push to the Model Hub.")
    parser.add_argument("--load_from_json", type=str, help="json file which specify script configurations")
    args = parser.parse_args()

    # load from json file
    if args.load_from_json:
        import json
        with open(args.load_from_json, "r") as f:
...
def main():
    ...
    # Make one log on every process with the configuration debugging
    ...
    # Setup logging, we only want one process per machine to log things on the csreen.
    # accelerator.is_local_main_process is only True for one preocess per machine.
    ...
    logger.info("****** Runnging training *******")
    logger.info(f"  Num examples = {len(train_dataset)}  ")
    logger.info(f"  Num epochs = {args.num_train_epochs}")
    logger.info(f"  Instantaneous batch size per device = {args.per_device_train_batch_size}")
    logger.info(f"  Gradient Accumulation steps = {args.gradient_accumulation_steps}")
    logger.info(f"  Total Opimtization steps = {args.max_train_steps}")

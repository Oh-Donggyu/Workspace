...
class ColaModel(LightningModule):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        outputs = self.bert(input_ids, attention_mask, labels)    

        return outputs

    def training_step(self, batch, batch_idx):
        # self.forward (input_ids, atteention_mask) 
        #                        -> logits: tensor[batch, num_classes], loss: tensor
        logits, loss = self.forward(
            self, batch["input_ids"], batch["attention_mask"], labels=batch["label"]
        )
    ...
    def validation_step(self, batch, batch_idx):
        labels = batch["label"]
        outputs = self.forward(batch["input_ids"], batch["attention_mask"], labels=labels)
        ...
        self.log("valid/acc", val_acc, prog_bar=True, on_step=True)
        ...
        self.log("valid/preicison_micro", precision_micro, prog_bar=True, on_step=True)
    ...
    def validation_epoch_end(self, outputs):
        ...
        logits = torch.cat([x["labels"] for x in outputs])
        preds = torch.argmax(logits, dim=1)
